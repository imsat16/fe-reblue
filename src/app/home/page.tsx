...
const HomePages = () => {
  ...
  React.useEffect(() => {
    ...
    const calculatedTotalWeight = data.reduce((acc, currentItem) => acc + currentItem.weight, 0);
        ...
        >
          <div className="flex flex-col gap-4">
            {locList?.map((_:any,i:number)=>{
              return(
                <div key={i}>
                  <p onClick={()=>{setSelectedLoc(_), setOpen(false)}} className='cursor-pointer border bg-white p-2 rounded-lg'>{_.address}</p>
                </div>
              )
            })}
          </div>
